...
namespace App\Types\XML {
    ...
    class LibXmlParser implements BaseXMLParser
    {
        ...
        private ?XMLParserEntryTypeEnum $entryType;
        private ?XMLParserEntryTypeEnum $nextEntryType;
        /**
         * @var string[] $data
         */
        private array $data;
        ...
        private XMLParserEventsInterface $events;

        private XMLParserPhaseEnum $phase;

        private bool $useLibxmlInternalErrors;

        public function setEvents(XMLParserEventsInterface $events): void
        {
            $this->events = $events;
        }

        public static function create(XMLParserEventsInterface $events): static
        {
            return new static($events);
        }

        private function __construct(XMLParserEventsInterface $events)
        {
            /**
            ...
            $this->events = $events;
            $this->phase = XMLParserPhaseEnum::NONE;
        ...
        {
            $this->updateEntryType(XMLParserEntryTypeEnum::ELEMENT_VALUE);
        ...
        private function otherConstructsHandler(XMLParser $parser, string $data): void
        {
            if (Str::startsWith($data, '<!--') && Str::endsWith($data, '-->')) {
                ...
                $this->unsupportedConstructHandler($parser,$data,XMLUnsupportedConstructTypeEnum::UNKNOWN_CONSTRUCT);
            }
        }

        private function unsupportedConstructHandler(XMLParser $parser, mixed $data,XMLUnsupportedConstructTypeEnum $type): void{
            $this->updateEntryType(XMLParserEntryTypeEnum::UNSUPPORTED_ENTITY);
        ...
        {
            $this->updateEntryType(XMLParserEntryTypeEnum::COMMENT);
        ...
        private function notationDeclHandler(
            ...
            $this->unsupportedConstructHandler($parser,[
                ...
                $public_id,
                $notation_name
            ], XMLUnsupportedConstructTypeEnum::NOTATION_DECLARATION);
        }
        ...
        private function startNamespaceHandler(XMLParser $parser, string|false $prefix, string $uri): void
        {
            $this->unsupportedConstructHandler($parser, [
                $prefix,
                $uri
            ], XMLUnsupportedConstructTypeEnum::START_NAMESPACE_DECLARATION);
        }
        ...
        private function unparsedEntityHandler(
            ...
            $this->unsupportedConstructHandler($parser, [
                ...
                $public_id,
                $notation_name
            ], XMLUnsupportedConstructTypeEnum::UNPARSED_ENTITY_DECLARATION);
        }
        ...
        private function processingInstructionHandler(XMLParser $parser, string $target, string $data): void
        {
            $this->unsupportedConstructHandler($parser, [
                $target,
                $data
            ], XMLUnsupportedConstructTypeEnum::PROCESSING_INSTRUCTION);
        }
        ...
        private function externalEntityRefHandlerhandler(
            ...
            $this->unsupportedConstructHandler($parser, [
                ...
                $system_id,
                $public_id
            ], XMLUnsupportedConstructTypeEnum::EXTERNAL_ENTITY_REFERENCE);
            return false;
        }

        private function isByteIndexValid(?XMLParserEntryTypeEnum $entryType, int $byteIndex)
        {
            $isByteIndexValid = false;//$this->dataByteIndex >= $byteIndex;
            if ($entryType === XMLParserEntryTypeEnum::ELEMENT_START) {
               ...
               // echo "ELEMENT START CHAR: '$char'";
                ...
                $entryType === XMLParserEntryTypeEnum::ELEMENT_END
                || $entryType === XMLParserEntryTypeEnum::COMMENT
            ) {
                $isByteIndexValid = $this->getCharAtByteIndex($byteIndex - 1) === '>';
            }
            // XMLParserEntryTypeEnum::ELEMENT_VALUE
            else if ($entryType === XMLParserEntryTypeEnum::ELEMENT_VALUE) {
                if ($isByteIndexValid = $this->positionIsValid()) {
                   ...
                   // echo "\nTHIS WONT WORK ";
                    if($this->entryType === XMLParserEntryTypeEnum::ELEMENT_START){
        ...
        private function updateEntryType(XMLParserEntryTypeEnum $entryType)
        {
            // dump("Update entry type {$entryType->name}");
            // if ($this->entryType === null && $entryType !== XMLParserEntryTypeEnum::ELEMENT_START) {
        ...
        public function getPos(?int &$column, ?int &$line, ?int &$byteIndex): void
        {
            ...
            <element>   </element>
            */
            if ($this->entryType === XMLParserEntryTypeEnum::UNSUPPORTED_ENTITY) {
            ...
            if (
                $this->nextEntryType === XMLParserEntryTypeEnum::ELEMENT_END
                || $this->nextEntryType === XMLParserEntryTypeEnum::ELEMENT_START
                || $this->nextEntryType === XMLParserEntryTypeEnum::COMMENT
            ) {
                $this->shiftData();
